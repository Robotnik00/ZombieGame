	...
	public void performAction()
	{
		...
		if(collisions.length > 0)
		{
			if(!collisions[0].isStatic()) // object is colliding with another object
			{
				...
				obj.setTranslationalVelocity(v1);
				
				v1.x = u1.x;
				v1.y = u1.y;
				v2.x = u2.x;
				v2.y = u2.y;
				v2.scale((obj.getMass() - collisions[0].getMass())/(obj.getMass()+collisions[0].getMass()));
				v1.scale(2*collisions[0].getMass()/(obj.getMass() + collisions[0].getMass()));
				Vector2f.add(v1, v2, v1);
				collisions[0].setTranslationalVelocity(v1);
